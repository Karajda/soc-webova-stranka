...
from methods.pomocne_funkce import spravne_desetinne, a, REAL_E

def vypocet(typ_limitu, hodnota_limitu):    #Výpočet Eulerova čísla pomocí řetězového zlomku
    ...
    pred_pred_citatel, pred_citatel = mpf(0), mpf(1)        #Určení prvních hodnot řetězového zlomku v čitateli
    pred_pred_jmenovatel, pred_jmenovatel = mpf(1), mpf(0)  #Určení prvních hodnot řetězového zlomku ve jmenovateli
    krok = 0    #Vyznačuje iterace
    e = mpf(0)  #Průběžná hodnota Eulerova čísla
    ...
    while True:
        ...
        a_krok = mpf(a(krok))
        citatel = a_krok * pred_citatel + pred_pred_citatel
        jmenovatel = a_krok * pred_jmenovatel + pred_pred_jmenovatel
        e = citatel / jmenovatel

        #Posun hodnot pro další iteraci
        pred_pred_citatel, pred_citatel = pred_citatel, citatel
        pred_pred_jmenovatel, pred_jmenovatel = pred_jmenovatel, jmenovatel
        ...
        ulozit = False
        if typ_limitu == 'iterace' and (krok + 1) >= index_mereni * interval_iteraci:
            ulozit = True
        ...
        if ulozit:
            aktualni_presnost = int(spravne_desetinne(e, REAL_E))    #Zjištění počtu správných desetinných míst
            
            #Výpočet rychlosti růstu konvergence podle typu limitu
            if typ_limitu == 'iterace':
                rozdil_kroku = ((krok + 1) - predchozi_krok)
            ...
            osa_x.append(int(krok + 1) if typ_limitu == 'iterace' else round(cas_vypoctu, 3))
            ...
            predchozi_presnost, predchozi_krok, predchozi_cas = aktualni_presnost, krok + 1, cas_vypoctu
            index_mereni += 1
    ...
    if not osa_x or osa_x[-1] != posledni_hodnota:
        posledni_presnost = int(spravne_desetinne(e, REAL_E))
