...
def vypocet(typ_limitu, hodnota_limitu):    #Výpočet čísla pí pomocí Chudnovského algoritmu
    ...
    suma_pi_obracena = mpf(0)   #Průběžná hodnota obrácené hodnoty pí (1/pi)
    pi = mpf(0)    #Průběžná hodnota čísla pí

    fac_k = mpf(1)      #Průběžná hodnota k!
    fac_3k = mpf(1)     #Průběžná hodnota 3k!
    fac_6k = mpf(1)     #Průběžná hodnota 6k!
    znamenko = mpf(1)   #Průběžné znaménko

    konst_nas = mpf(640320) ** mpf(1.5) #Konstantní hodnota 640320^(3/2)
    konst_nasobitel = mpf(640320) ** 3  #Konstantní hodnota 640320^3
    konst_a = mpf(545140134)    #Konstantní hodnota 545140134
    konst_b = mpf(13591409)     #Konstantní hodnota 13591409
    dvanact = mpf(12)           #Převedení čísla 12 na přesnost nastavenou výše, aby nedocházelo ke konverzi datových typů každou iteraci
    ...
    while True:
        ...
        ak = znamenko * fac_6k / (fac_3k * fac_k**3 * konst_nas)
        term = ak * (konst_a * krok + konst_b)
        suma_pi_obracena += term
        pi = 1 / (dvanact * suma_pi_obracena)
        
        #Posun hodnot pro další iteraci
        krok += 1
        fac_k *= krok
        fac_3k *= (3*krok - 2) * (3*krok - 1) * (3*krok)
        fac_6k *= (6*krok - 5) * (6*krok - 4) * (6*krok - 3) * (6*krok - 2) * (6*krok - 1) * (6*krok)
        konst_nas *= konst_nasobitel
        znamenko = -znamenko
