...
def vypocet(typ_limitu, hodnota_limitu):    #Výpočet čísla pí pomocí metody Monte Carlo
    ...
    uvnitr = 0      #Počet bodů uvnitř kruhu
    ...
    while True:
        ...
        #Výpočet jedné iterace (generování náhodných souřadnic x a y)
        x = random.uniform(-1, 1)   
        y = random.uniform(-1, 1)
        if x**2 + y**2 <= 1:
            uvnitr += 1
        
        pi = ctyri * uvnitr / krok
